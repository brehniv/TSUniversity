...
function triangle(value1, type1, value2, type2) {
    ...
    var anglesList = ["adjacent angle", "opposite angle", "angle"];
    ...
    if (anglesList.includes(type1)) {
        if (value1 < 0 || value1 > 90) {
            return "failed: incorrect angle ".concat(type1);
        }
    }
    if (anglesList.includes(type2)) {
        if (value2 < 0 || value2 > 90) {
            return "failed: incorrect angle ".concat(type2);
        }
    }
    ...
    console.log("a = ".concat(round2(a), "\nb = ").concat(round2(b), "\nc = ").concat(round2(c), "\nalpha = ").concat(round2(alpha), "\u00B0\nbeta = ").concat(round2(beta), "\u00B0"));
    return "success";
}
